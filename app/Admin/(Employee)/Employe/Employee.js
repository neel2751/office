...
const Employee = ({ page }) => {
  ...
  const employeData = useCallback(async () => {
    if (typeof window !== "undefined") {
      setLoading(true);
      try {
        const response = await getAllEmployees(searchDebounce, filter);
        if (response.count === 0)
        ...
        if (response.status) {
          setFilter({ ...filter, totalCount: response.count });
  ...
  return (
    <div className="h-full w-full mt-16 bg-gray-50 relative overflow-y-auto lg:ml-64">
      ...
      {/* <select
        id="location"
        name="location"
        class="sm:leading-6 sm:text-sm outline-none ring-inset text-neutral-800 pr-10 pl-3 py-1.5 border-none rounded-md max-w-max block mt-2 bg-[url('https://www.svgrepo.com/show/491603/chevron-up-chevron-down.svg')] bg-[center_right_0.5rem] bg-no-repeat bg-[length:1em_1em] appearance-none cursor-pointer"
      >
        {["All", "Weekly", "Monthly"].map((item) => (
          <option value={item} className="p-1 hover:bg-neutral-200 rounded-md">
            {item}
          </option>
        ))}
      </select> */}
      ...
      {/* <Button
        text={"Next Week"}
        type="button"
        cls={"bg-neutral-800 text-white rounded-md  py-2 px-4"}
        onclick={testNextWeek}
      /> */}
          ...
          <div className="mb-1 w-full">
            <div className="sm:flex">
              <div className="sm:flex items-center sm:divide-x sm:divide-gray-100 mb-3 sm:mb-0">
                <Search
                  onChange={setSearch}
                  placeholder={`Search ${page.split("/")[2]}`}
                />
              </div>
            ...
            <div className="bg-[url('/images/bgpattern.svg')] text-center text-neutral-700 mt-10 p-2 h-[27rem] flex flex-col items-center justify-center border border-dashed border-neutral-300 rounded-md">
              ...
              <div>
                <ButtonLink
                  title={"Add  Employee"}
                  cls={
                    "flex justify-center items-center bg-gradient-to-r from-neutral-900 via-neutral-800 to-neutral-950 text-white shadow-md p-2.5 rounded-lg text-sm gap-x-2 mt-2"
                  }
...
const EditEmployeModel = ({ data, isOpen, onRequestClose, onSucessEdit }) => {
  return (
    <EmployeeModelCard
      cls={"max-w-4xl"}
      isOpen={isOpen}
