...
const SiteProject = ({ page }) => {
  ...
  const getData = async (siteId) => {
    console.log(siteId);
    const response = await getTotalCostMonthWise(siteId);
                    ...
                    currentData.map((item) => (
                      <tr key={item._id}>
                        ...
                        <td className="p-4 whitespace-nowrap space-x-2">
                          ...
                          />
                          <TableAction
                            link={`/Admin/SiteProject/${item._id}/overview`}
                            svg={<View className="text-neutral-200 size-5" />}
                            cls="text-white bg-neutral-700 hover:bg-neutral-900 focus:ring-4 focus:ring-red-300"
